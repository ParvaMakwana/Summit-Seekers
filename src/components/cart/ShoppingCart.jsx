          ...
          {items.length > 0 && (
            <div className="border-t border-gray-700 px-4 py-6">
              <div className="flex items-center justify-between mb-4">
                <span className="text-gray-300">Total ({getCartItemsCount()} items)</span>
                <span className="text-2xl font-bold text-white">${getCartTotal().toFixed(2)}</span>
              </div>
              <Link
                to="/checkout"
                onClick={onClose}
                className="w-full bg-orange-500 hover:bg-orange-600 text-white py-3 px-4 rounded-lg font-semibold transition-colors duration-200 block text-center"
              >
                Proceed to Checkout
              </Link>
            </div>
